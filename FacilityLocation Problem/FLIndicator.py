...
for i in range(nbFacilities):
    mdl.add_constraint(mdl.indicator_constraint(x[i], mdl.sum(y[i,j] * demandCustomer[j] for j in range(nbCustomer)) <= maximumProduction[i]))
    mdl.add_constraint(mdl.indicator_constraint(x[i], mdl.sum(y[i,j] * demandCustomer[j] for j in range(nbCustomer)) <= 0, active_value=0))
