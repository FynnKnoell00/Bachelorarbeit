...
def build_model(amount_jobs, amount_machines, machine_sequences, proc_times, transposed_matrix_procTime):
    ...
    # Create a Dictionary for the binary variables
    ...
    y = mdl.integer_var_matrix(amount_machines, amount_jobs, name='y')
        # Binary variables that indicate whether y[m1][j] goes before y[m2][l]
    ...
    # Set the value for bigM big enough
    bigM = 9999
        ...
        for j in range(amount_jobs):
            for l in range(amount_jobs):
                if j != l:
                    ...
                    mdl.add_constraint(y[i, j] + transposed_matrix_procTime[m1][j]
                                       - bigM * (1 - b[i, j, i, l])
                                       <= y[i, l])
                    mdl.add_constraint(y[i, l] + transposed_matrix_procTime[m1][l]
                                       - bigM * b[i, j, i, l]
                                       <= y[i, j])
...
def solve_and_save_solution(model):
    ...
    with open("solution_Value.txt", "a") as solfile:
        solfile.write("statusMIP = " + str(model.solve_details.status) + " = " + str(solution.get_objective_value()) +  ",     ")
