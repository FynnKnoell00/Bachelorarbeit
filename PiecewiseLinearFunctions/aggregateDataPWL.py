...
def calculate_time_differences(func_times, linear_times, sos_times):
    ...
    differences1_2 = [abs(t1 - t2) for t1, t2 in zip(func_times, linear_times)]
    differences1_3 = [abs(t1 - t3) for t1, t3 in zip(func_times, sos_times)]
    differences2_3 = [abs(t2 - t3) for t2, t3 in zip(linear_times, sos_times)]

    # Calculate the average differences by dividing the sum by the number of elements
    avg_diff_1_2 = round(sum(differences1_2) / len(differences1_2), 3)
    avg_diff_1_3 = round(sum(differences1_3) / len(differences1_3), 3)
    avg_diff_2_3 = round(sum(differences2_3) / len(differences2_3), 3)
...
def compare_and_save_stats(func_times, linear_times, sos_times, file_path, timelimit):
    ...
    with open(file_path, "w") as file:
        file.write(f"Simulated {100} times\n\n")
        ...
        if func_faster_count > 0:
            file.write(f"Smallest Difference to Linear when Function was fastest: {min(time_diffs_funclinear):.3f} sec\n")
            file.write(f"Smallest Difference to SOS when Function was fastest: {min(time_diffs_funcsos):.3f} sec\n")
            file.write(f"Biggest Difference to Linear when Function was fastest: {max(time_diffs_funclinear):.3f} sec\n")
            file.write(f"Biggest Difference to SOS when Function was fastest: {max(time_diffs_funcsos):.3f} sec\n")

        #
        if linear_faster_count > 0:
            file.write(f"Smallest Difference to Function when Linear was fastest: {min(time_diffs_funclinear):.3f} sec\n")
            file.write(f"Smallest Difference to SOS when Linear was fastest: {min(time_diffs_linearsos):.3f} sec\n")
            file.write(f"Biggest Difference to Function when Linear was fastest: {max(time_diffs_funclinear):.3f} sec\n")
            file.write(f"Biggest Difference to SOS when Linear was fastest: {max(time_diffs_linearsos):.3f} sec\n")
        
        # 
        if sos_faster_count > 0:
            file.write(f"Smallest Difference to Function when SOS was fastest: {min(time_diffs_funcsos):.3f} sec\n")
            file.write(f"Smallest Difference to Linear when SOS was fastest: {min(time_diffs_linearsos):.3f} sec\n")
            file.write(f"Biggest Difference to Function when SOS was fastest: {max(time_diffs_funcsos):.3f} sec\n")
            file.write(f"Biggest Difference to Linear when SOS was fastest: {max(time_diffs_linearsos):.3f} sec\n")
